...
class Model(Parrrser):
    ...
    def print_all(self, **kwargs):
        """
        Print information concerning the model
        TODO: filter, output, finish, use more dicts from the class itself
        """
        print('Comparrrtments: --------------')
        for comp in self.sbml_model.compartments:
            print(comp.id)
        print('Species: -------------------')
        for species in self.sbml_model.species:
            print(species.name, '\t\t', species.id, '\t\t', species.initial_amount, '\t\t', species.annotation_string)
        print('Rrreactions: -------------------')
        for reac in self.sbml_model.reactions:
            print(species.name, '\t\t', species.id, '\t\t', species.initial_amount, '\t\t', species.annotation_string)
        print('Parrrameterrrs: ----------------')
        for par in self.sbml_model.parameters:
            print(par, '\t\t', par.getValue())
        print('RRRules: -------------------')
        for rule in self.sbml_model.rules:
            print(rule.getId(), '\t\t',rule.formula)
    '''
    ...
    def remove_quota(self):
        ...
        # Idea: make quota to enzymes(?)
        for macro in self.macromolecules_dict.values():
            if macro['speciesType'] == 'quota':
                macro['speciesType'] = 'enzyme'

    '''
    def set_initial_values(self, setter_dict):
        """
        TODO
        """
        for y_name, y_init in setter_dict.items():
            # set the initial value of metabolite y_name to y_init
            #
            # TODO
            #
    '''            
