...
from ..util.linalg import dkron, shape_of_callable, is_nonzero_array
...
def cp_rk_linprog_v(matrices, rkm, tgrid, varphi=0.0,
    ...
    TODO
     - refactor: * don't allocate matrices all here: use add_constraints(?)
    ...
    # Boundary Values / Global Constraints  ==================================
    aeq4_y = sp.hstack([matrices.matrix_start, sp.csr_matrix((n_bndry, (n_steps-1)*n_y)),
                        matrices.matrix_end], format='csr')
    aeq4_k = sp.csr_matrix((n_bndry, n_allk))
    aeq4_u = sp.csr_matrix((n_bndry, n_allu))
    aeq4_p = matrices.matrix_bndry_p
    beq4 = dkron(np.array([[1.0]]), matrices.vec_bndry, np.array([[1.0]]), out_type='np')# TODO: type check improvement
    # add the integral constraint parts
    default_t = tgrid[0]
    if is_nonzero_array(matrices.bmatint_y, default_t=default_t) or \
          is_nonzero_array(matrices.bmatint_u, default_t=default_t):
        #
        for k in range(n_bndry):
            aeq4_y_t, aeq4_k_t, aeq4_u_t, _ = \
                 _inflate_integral_vectors(matrices.bmatint_y[k, :].T,
                                           sp.csr_matrix((n_y, 1)),
                                           sp.csr_matrix((n_y, 1)),
                                           matrices.bmatint_u.T,
                                           sp.csr_matrix((n_p, 1)), rkm, tgrid)
            # FIXME: Changing sparsity structure here: catch and/or build matrices instead of
            #   updating like this.
            aeq4_y[k,:] += aeq4_y_t.T
            aeq4_k[k,:] += aeq4_k_t.T
            aeq4_u[k,:] += aeq4_u_t.T
    ...
    # Assembly of LP =========================================================
    #print(f_y.shape, f_k.shape, f_u.shape, f_p.shape)
    ...
    if model.status == lp_wrapper.OPTIMAL:
        ...
        #
        obj_result = model.get_objective_val()
        #
        return {'tgrid': tgrid,
...
'''
class ControlNotParameterizedError(UnboundLocalError):
    """
    Control could not be calculated.
    """

class ControlWithoutGridError(Exception):
    """
    not enough data to evaluate control
    """
'''
...
class SingleControl():
    """
    Not necessarily differentiable time-dependent variable associated with a way to parameterize
    and evaluate
    """
    def __init__(self, tgrid, eval_mode='maingrid'):
        self.tgrid = tgrid
        self.pars = None
        self.eval_mode = eval_mode


    def __add__(self, other_controls):
        return other_controls.append(self)


    def evaluate(self, time):
        """
        Compute value at a time point
        """
        if self.eval_mode == 'maingrid':
            raise ControlWithoutGridError
        if self.pars:
            return self._evaluate(time)
        raise ControlNotParameterizedError


    def parameterize(self, pars):
        """
    ...
    def _evaluate(self, time):
        """
        compute the value at a point in time w/o additional checks
        """
'''
...
class Controls():
    """
    Collector of SingleControl-instances (@QUESTION: NEEDED???)
    """
    def __init__(self, control_list):
        self._has_data = False
        self.control_list = control_list
        # -1: unknown, 0 False, 1 True
        self._details = {'single_grid': -1, 'single_eval_mode': -1}
        #
        #self.nodes = nodes # TODO: actually: main grid and minor grid, potentially even multiple
        # different grids....!!!!!

    def evaluate(self, time):
        """
        Compute value a time point
        """
...
class LinOCProb():
    """
    (Abs-) Linear Optimal Control Problem
    """
    def __init__(self, mats, **kwargs):
        self.name = kwargs.get('name', 'Linear OC problem')
        self.matrrrices = mats
        self.is_up_to_date = False

    def update(self):
        """
        check and reset potentially outdated values
        """
    ...
    def check_dimensions(self):
        ...
        Verify values
        """
        self.matrrrices.check_dimensions()


class LinOC():
    """
    (Abs-) Linear Optimal Control Problem Handler to solve OCs with CP or CVP using linear
    optimization
    """
    def __init__(self, **kwargs):
        self.name = kwargs.get('name', 'Linear OC problem')
        self.lin_oc_prob = kwargs.get('oc_prob', None)
        self.is_up_to_date = False
        self.oc_scheme = kwargs.get('oc_scheme', RungeKuttaPars())
        self.quad_scheme = kwargs.get('quad_scheme', RungeKuttaPars())
    ...
    def update(self):
        """
        update the matriccces and the underlying lp instance
        """
        if not self.is_up_to_date:
            self.lin_oc_prob.update()
            self.lpinstance.update()
        else:
    ...
    def solve(self):
        ...
        Solve the OC
        """
        print('Solving ' + self.lin_oc_prob.name + ' with ' + self.name)
