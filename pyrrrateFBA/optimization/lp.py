...
if DEFAULT_SOLVER == 'gurobi':
    try:
        import gurobipy
        # Module constants
        INFINITY = gurobipy.GRB.INFINITY # pylint: disable=E1101
        OPTIMAL = gurobipy.GRB.OPTIMAL # pylint: disable=E1101
        MINIMIZE = gurobipy.GRB.MINIMIZE # pylint: disable=E1101
        #
        # FeasibilityTol,Primal feasibility tolerance  Type: 	double	Default value: 	1e-6
        #      Minimum value: 	1e-9	Maximum value: 	1e-2
        # IntFeasTol, Integer feasibility tolerance 	Type: 	double 	Default value: 	1e-5
        #      Minimum value: 	1e-9 	Maximum value: 	1e-1
        # MarkowitzTol (Bigger is better), Type: 	double, 	Default value: 	0.0078125
        #      Minimum value: 	1e-4 	Maximum value: 	0.999
        # NumericFocus,  	Type: 	int  	Default value: 	0
        #      Minimum value: 	0  	Maximum value: 	3 (higher: more accurate)
        # ObjScale Objective scaling 	Type: 	double 	Default value: 	0.0
        #      Minimum value: 	-1 	Maximum value: 	Infinity
        # OptimalityTol Dual feasibility tolerance Type: 	double 	Default value: 	1e-6
        #      Minimum value: 	1e-9 	Maximum value: 	1e-2
        #
    except ImportError:
        DEFAULT_SOLVER = 'soplex'
...
class LPModel():
    ...
    def optimize(self):
        ...
        if self.solver_name == 'gurobi':
            ...
            tmp = self.solver_model.simplex(msg_lev=self.solver_model.MSG_ALL,
                                            presolve=True) # FIXME!!!!! integer/intopt nur fuer
                                                  # MILPs
            #print(tmp)
            ...
            if self.solver_model.status != 'opt':
                return None  # If no relaxed sol., no exact sol.
    ...
    def poor_mans_feasiblizer(self, mode=2, fixed_leq_indices=None):
        """
        Quick-and-Dirty
        """
        if self.solver_name == 'gurobi':
            ...
            if mode == 1:
                pass
            # Go through the inequality constraint (not in fixed_leq_indices) and just check
            # feasibility when they are removed, TODO
            #allvars = self.solver_model.getVars()
            #ubpen = [1.0]*self.solver_model.numVars
            """
             If you specify relaxobjtype=0, the objective of the feasibility relaxation is to
               minimize the sum of the magnitudes of the bound and constraint violations.
             If you specify relaxobjtype=1, the objective of the feasibility relaxation is to
               minimize the sum of the squares of the bound and constraint violations.
             If you specify relaxobjtype=2, the objective of the feasibility relaxation is to
               minimize the total number of bound and constraint violations.
            """
            # (2) ---------------------------------------------------------------------------------
            if mode == 2:
                # Constraint relaxation from deviation in constraint violations
                #self.solver_model.feasRelax(1, False, allvars, None, ubpen, None, None)
                #self.solver_model.feasRelaxS(0, False, False, True)
                self.solver_model.feasRelaxS(1, False, False, True)
                #self.solver_model.feasRelaxS(2, False, False, True)
                self.solver_model.optimize()
                return self.solver_model.x
            # (3) ---------------------------------------------------------------------------------
            if mode == 3:
                # irreducible inconsistent subsystem
                tmp = self.solver_model.computeIIS()
                return tmp # IISConstr, IISLB, IISUB, IISSOS, IISQConstr, and IISGenConstr.
        else:
...
def _sparse_model_setup_gurobi(model, fvec, amat, bvec, aeqmat, beq, lbvec,
    ...
    #
    #
    qnd_set_tight_tolerances = True#False
    if qnd_set_tight_tolerances:
        #print('Tight tolerances!!')
        # FeasibilityTol,Primal feasibility tolerance  Type: 	double	Default: 	1e-6	
        #      Minimum: 	1e-9	Maximum value: 	1e-2
        model.setParam('FeasibilityTol', 1.0e-9)
        # IntFeasTol, Integer feasibility tolerance 	Type: 	double 	Default value: 	1e-5 	
        #      Minimum value: 	1e-9 	Maximum value: 	1e-1
        # MarkowitzTol (Bigger is better), Type: double, Default: 0.0078125, 
        #      Minimum: 1e-4, Maximum: 	0.999
        model.setParam('MarkowitzTol', 0.999)
        # NumericFocus,  	Type: 	int  	Default value: 	0  	
        #      Minimum value: 	0  	Maximum value: 	3 (higher: more accurate)
        ...
        # OptimalityTol Dual feasibility tolerance Type: 	double 	Default value: 	1e-6 	
        #      Minimum value: 	1e-9 	Maximum value: 	1e-2
        model.setParam('OptimalityTol', 1.0e-9)
        #
    #
...
def _sparse_model_setup_glpk(model, fvec, amat, bvec, aeqmat, beq, lbvec,
    ...
    for i, col in enumerate(model.cols):
        col.name = variable_names[i]
        col.bounds = lbvec[i], ubvec[i]
        if i < n_x: # FIXME: Check this!!!!!!!!!!
            col.kind = float
        else:
            col.kind = bool
